...
The default configuration applies these extensions to `SilverStripe\Model\ModelData` only. If you wish to add them to other `SilverStripe\Core\Extensible` classes that aren't subclasses of the former you can use the following configuration:
...
```diff
if (\SilverStripe\Model\ModelData::has_extension(Foo::class, FooExtension::class)) {
